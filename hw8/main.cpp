...
static GLint useFragmentUniformPos;
static int useFragment1 = 1;
...
static void readShaders() {
   ...
   useFragmentUniformPos = glGetUniformLocation(shaderProgram, "useFragment");
   ...
   glUniform1i(useFragmentUniformPos, useFragment1);
}
...
int main(int argc, char *argv[]) {
   ...
   // parse x and y resolutions
   int xRes = atoi(argv[1]);
   int yRes = atoi(argv[2]);
   
   // check if arguments are invalid
   // note: atoi returns 0 if invalid, so can use that to our advantage
   if (xRes < 100 || yRes < 100)
   {
      cerr << "Invalid input resolutions. Use at least 100 for each." << endl;
      return -1;
   }
